...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.ai.recommendation",
  });

  return {
    title: t("mainSection.title") || "Recommendation Systems",
    description:
      t("mainSection.description") ||
      "Enhance user experience with personalized recommendations powered by AI.\nDeliver tailored suggestions to increase satisfaction and repeat purchases",
  };
}

const servicesItems = en.services.ai.recommendation.items.map((_, index) => ({
  count: `0${index + 1}`,
...
const RecommendationSystems: FC<{
  params: { locale: string };
}> = async ({ params: { locale } }) => {
  const t = await getTranslations({
    locale,
