...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.ai.imageRecognition",
  });

  return {
    title: t("mainSection.title") || "Image Recognition and Analysis",
    description:
      t("mainSection.description") ||
      "Automate visual data processing for quality control, security, and inventory management.\nReduce errors and save time by leveraging AI for fast and accurate image analysis.",
  };
}

const servicesItems = en.services.ai.imageRecognition.items.map((_, index) => ({
  count: `0${index + 1}`,
...
const ImageRecognitionAndAnalysis: FC<{
  params: { locale: string };
}> = async ({ params: { locale } }) => {
  const t = await getTranslations({
    locale,
...
export default ImageRecognitionAndAnalysis;
