...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.analytics.mainSection",
  });

  return {
    title: t("title") || "Analytics and Reporting Services",
    description:
      t("description") ||
...
const servicesItems = en.services.analytics.items.map((_, index) => ({
  count: `0${index + 1}`,
...
const ImageRecognitionAndAnalysis: FC<{ params: { locale: string } }> = async ({
  params: { locale },
}) => {
  const t = await getTranslations({ locale, namespace: "services.analytics" });
