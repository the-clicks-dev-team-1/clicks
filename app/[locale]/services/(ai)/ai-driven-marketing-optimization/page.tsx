...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.ai.aiMarketing",
  });

  return {
    title: t("mainSection.title") || "AI-Driven Marketing Optimization",
    description:
      t("mainSection.description") ||
      "Optimize campaigns and customer targeting with AI-powered tools for better ROI.\nDeliver highly targeted ads and maximize your marketing budget with AI insights.",
  };
}

const servicesItems = en.services.ai.aiMarketing.items.map((_, index) => ({
  count: `0${index + 1}`,
...
const AIDrivenMarketingOptimization: FC<{
  params: { locale: string };
}> = async ({ params: { locale } }) => {
  const t = await getTranslations({
    locale,
...
export default AIDrivenMarketingOptimization;
