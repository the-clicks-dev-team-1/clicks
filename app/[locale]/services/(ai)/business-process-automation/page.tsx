...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.ai.automation",
  });

  return {
    title: t("mainSection.title") || "Business Process Automation",
    description:
      t("mainSection.description") ||
      "Streamline repetitive tasks with AI-powered automation, saving time and reducing errors.\nBoost operational efficiency while eliminating human errors through intelligent process automation.",
  };
}

const servicesItems = en.services.ai.automation.items.map((_, index) => ({
  count: `0${index + 1}`,
...
const BusinessProcessAutomation: FC<{
  params: { locale: string };
}> = async ({ params: { locale } }) => {
  const t = await getTranslations({
    locale,
