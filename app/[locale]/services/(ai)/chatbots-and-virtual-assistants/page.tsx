...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.ai.chatbots",
  });

  return {
    title: t("mainSection.title") || "Chatbots and Virtual Assistants",
    description:
      t("mainSection.description") ||
      "Enhance customer experience with intelligent chatbots that provide instant, personalized support.\nDeliver 24/7 customer service without overburdening your staff.",
  };
}

const servicesItems = en.services.ai.chatbots.items.map((_, index) => ({
  count: `0${index + 1}`,
...
const ChatbotsAndVirtualAssistants: FC<{
  params: { locale: string };
}> = async ({ params: { locale } }) => {
  const t = await getTranslations({
    locale,
