...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.ai.cybersecurity",
  });

  return {
    title: t("mainSection.title") || "AI-Enhanced Cybersecurity",
    description:
      t("mainSection.description") ||
      "Strengthen your cybersecurity with AI tools that detect and prevent threats in real time.\nStay protected with intelligent systems that identify vulnerabilities and respond instantly.",
  };
}

const servicesItems = en.services.ai.cybersecurity.items.map((_, index) => ({
  count: `0${index + 1}`,
...
const AIEnhancedCybersecurity: FC<{
  params: { locale: string };
}> = async ({ params: { locale } }) => {
  const t = await getTranslations({
    locale,
