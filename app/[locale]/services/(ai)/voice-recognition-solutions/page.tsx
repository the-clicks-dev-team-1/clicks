...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.ai.voice",
  });

  return {
    title: t("mainSection.title") || "Voice Recognition Solutions",
    description:
      t("mainSection.description") ||
      "Enable voice-based solutions for customer support, accessibility, and automation.\nStreamline interactions and enhance accessibility with cutting-edge voice recognition technology.",
  };
}

const servicesItems = en.services.ai.voice.items.map((_, index) => ({
  count: `0${index + 1}`,
...
const VoiceRecognitionSolutions: FC<{
  params: { locale: string };
}> = async ({ params: { locale } }) => {
  const t = await getTranslations({
    locale,
...
export default VoiceRecognitionSolutions;
