...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.blockchain.consulting",
  });

  return {
    title: t("mainSection.title") || "Blockchain Consulting",
    description:
      t("mainSection.description") ||
      "Expert guidance to help businesses identify blockchain opportunities and implement the right solutions.\nEmpower your business with strategies that maximize blockchain's potential while minimizing risks.",
  };
}

const servicesItems = en.services.blockchain.consulting.items.map(
  (_, index) => ({
    count: `0${index + 1}`,
...
const BlockchainConsulting: FC<{ params: { locale: string } }> = async ({
  params: { locale },
}) => {
  const t = await getTranslations({
    locale,
