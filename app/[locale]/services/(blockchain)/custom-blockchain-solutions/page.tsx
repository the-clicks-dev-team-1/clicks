...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.blockchain.customSolutions",
  });

  return {
    title: t("mainSection.title") || "Custom Blockchain Solutions",
    description:
      t("mainSection.description") ||
      "Tailored blockchain systems for your business, ensuring seamless integration and improved operations.\nSave time and reduce manual errors with automated blockchain processes. Enhance trust and transparency across all your business operations.",
  };
}

const servicesItems = en.services.blockchain.customSolutions.items.map(
  (_, index) => ({
    count: `0${index + 1}`,
...
const CustomBlockchainSolutions: FC<{ params: { locale: string } }> = async ({
  params: { locale },
}) => {
  const t = await getTranslations({
    locale,
      ...
      <div className="container md:px-10 mb-32 pt-32 md:pt-40">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          ...
          <div className="relative w-full h-64 md:h-96">
            <Image
              src="/images/blockchain/customSolutions.jpg"
