...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.blockchain.supplyChain",
  });

  return {
    title: t("mainSection.title") || "Supply Chain Blockchain Solutions",
    description:
      t("mainSection.description") ||
      "Optimize supply chains with blockchain for better transparency and operational efficiency.\nReduce processing time and prevent counterfeiting with automated transaction processing.",
  };
}

const servicesItems = en.services.blockchain.supplyChain.items.map(
  (_, index) => ({
    count: `0${index + 1}`,
...
const SupplyChainBlockchain: FC<{
  params: { locale: string };
}> = async ({ params: { locale } }) => {
  const t = await getTranslations({
    locale,
...
export default SupplyChainBlockchain;
