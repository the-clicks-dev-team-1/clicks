...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.blockchain.securityAudits",
  });

  return {
    title: t("mainSection.title") || "Blockchain Security Audits",
    description:
      t("mainSection.description") ||
      "Ensure the safety of your blockchain applications with comprehensive security audits.\nIdentify vulnerabilities and strengthen your system against potential hacks.",
  };
}

const servicesItems = en.services.blockchain.securityAudits.items.map(
  (_, index) => ({
    count: `0${index + 1}`,
...
const BlockchainSecurityAudits: FC<{ params: { locale: string } }> = async ({
  params: { locale },
}) => {
  const t = await getTranslations({
    locale,
      ...
      <div className="container md:px-10 mb-32 pt-32 md:pt-40">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          ...
          <div className="relative w-full h-96 md:h-full md:max-h-[800px]">
            <Image
              src="/images/blockchain/securityAudits.jpg"
...
export default BlockchainSecurityAudits;
