...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.blockchain.privateBlockchain",
  });

  return {
    title: t("mainSection.title") || "Private Blockchain Development",
    description:
      t("mainSection.description") ||
      "Develop private blockchain networks for secure and efficient internal operations.\nEnable enhanced privacy, complete data control, and scalability tailored to your enterprise needs.",
  };
}

const servicesItems = en.services.blockchain.privateBlockchain.items.map(
  (_, index) => ({
    count: `0${index + 1}`,
...
const PrivateBlockchainDevelopment: FC<{
  params: { locale: string };
}> = async ({ params: { locale } }) => {
  const t = await getTranslations({
    locale,
      ...
      <div className="container md:px-10 mb-32 pt-32 md:pt-40">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          ...
          <div className="relative w-full h-96 md:h-full md:max-h-[800px]">
            <Image
              src="/images/blockchain/privateBlockchain.jpg"
