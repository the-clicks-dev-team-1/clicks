...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.blockchain.nft",
  });

  return {
    title: t("mainSection.title") || "NFT Platform Development",
    description:
      t("mainSection.description") ||
      "Design and develop platforms for buying, selling, and managing NFTs and digital assets.\nEmpower businesses with branding opportunities and secure trading experiences.",
  };
}

const servicesItems = en.services.blockchain.nft.items.map((_, index) => ({
  count: `0${index + 1}`,
...
const NFTPlatformDevelopment: FC<{ params: { locale: string } }> = async ({
  params: { locale },
}) => {
  const t = await getTranslations({
    locale,
...
export default NFTPlatformDevelopment;
