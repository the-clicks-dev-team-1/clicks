...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.blockchain.cryptoWallet",
  });

  return {
    title: t("mainSection.title") || "Crypto Wallet Development",
    description:
      t("mainSection.description") ||
      "Develop secure and user-friendly cryptocurrency wallets for transactions and asset management.\nEnable seamless management of digital assets with multi-currency support and advanced security.",
  };
}

const servicesItems = en.services.blockchain.cryptoWallet.items.map(
  (_, index) => ({
    count: `0${index + 1}`,
...
const CryptoWalletDevelopment: FC<{ params: { locale: string } }> = async ({
  params: { locale },
}) => {
  const t = await getTranslations({
    locale,
      ...
      <div className="container md:px-10 mb-32 pt-32 md:pt-40">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          ...
          <div className="relative w-full h-64 md:h-96">
            <Image
              src="/images/blockchain/cryptoWallet.jpg"
...
export default CryptoWalletDevelopment;
