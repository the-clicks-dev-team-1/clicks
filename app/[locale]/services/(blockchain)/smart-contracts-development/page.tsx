...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.blockchain.smartContracts",
  });

  return {
    title: t("mainSection.title") || "Smart Contracts Development",
    description:
      t("mainSection.description") ||
      "Efficient and secure smart contracts to automate agreements and streamline transactions.\nReduce manual intervention and eliminate intermediaries while ensuring transparency and tamper-proof execution of agreements.",
  };
}

const servicesItems = en.services.blockchain.smartContracts.items.map(
  (_, index) => ({
    count: `0${index + 1}`,
...
const SmartContractsDevelopment: FC<{
  params: { locale: string };
}> = async ({ params: { locale } }) => {
  const t = await getTranslations({
    locale,
      ...
      <div className="container md:px-10 mb-32 pt-32 md:pt-40">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          ...
          <div className="relative w-full h-96 md:h-full md:max-h-[800px]">
            <Image
              src="/images/blockchain/smartContracts.jpg"
...
export default SmartContractsDevelopment;
