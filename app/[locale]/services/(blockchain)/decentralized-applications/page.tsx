...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.blockchain.dApps",
  });

  return {
    title: t("mainSection.title") || "Decentralized Applications (DApps)",
    description:
      t("mainSection.description") ||
      "Build secure and scalable DApps to enhance business operations and customer experiences.\nLeverage decentralized infrastructure for reliable and high-performing solutions",
  };
}

const servicesItems = en.services.blockchain.dApps.items.map((_, index) => ({
  count: `0${index + 1}`,
...
const DecentralizedApplications: FC<{ params: { locale: string } }> = async ({
  params: { locale },
}) => {
  const t = await getTranslations({
    locale,
...
export default DecentralizedApplications;
