...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "services.blockchain.training",
  });

  return {
    title: t("mainSection.title") || "Blockchain Training Programs",
    description:
      t("mainSection.description") ||
      "Comprehensive training programs to help your team understand and utilize blockchain effectively.\nEquip your workforce with the skills to leverage blockchain for long-term success.",
  };
}

const servicesItems = en.services.blockchain.training.items.map((_, index) => ({
  count: `0${index + 1}`,
...
const BlockchainTraining: FC<{ params: { locale: string } }> = async ({
  params: { locale },
}) => {
  const t = await getTranslations({
    locale,
...
export default BlockchainTraining;
