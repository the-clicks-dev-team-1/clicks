                ...
                >
                    {navItems.map((d, i) => (
                        <div key={`${d.label}-${i}`} className="relative group">
                            <Link href={d.link ?? "#"} className="flex items-center gap-2 text-neutral-400 hover:text-white transition-all" onClick={handleLinkClick}>
                                <span>{d.label}</span>
                                {d.children && (
                            ...
                            {d.children && (
                                <div className="absolute left-0 top-full hidden group-hover:block flex-col bg-black/90 text-white rounded-lg shadow-md py-3 transition-all">
                                    {d.children.map((ch, j) => (
                                        <Link key={`${ch.label}-${j}`} href={ch.link ?? "#"} className="flex items-center px-4 py-2 hover:bg-sky-400" onClick={handleLinkClick}>
                                            {ch.iconImage && (
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex justify-end md:hidden">
            <div className="h-full w-[75%] bg-black px-4 py-4 overflow-y-auto">
                ...
                <div className="flex flex-col gap-2 mt-4">
                    {navItems.map((d, i) => (
                        <SingleNavItem key={`${d.label}-${i}`} {...d} closeSideMenu={closeSideMenu} />
                    ))}
                </div>
    ...
    return (
        <div ref={animationParent} className="relative">
            {d.children ? (
                <>
                    ...
                    {isItemOpen && (
                        <div className="flex flex-col bg-black text-white rounded-lg shadow-md py-3">
                            {d.children.map((ch, i) => (
                                <Link key={`${ch.label}-${i}`} href={ch.link ?? "#"} className="flex items-center px-4 py-2 hover:bg-sky-400" onClick={d.closeSideMenu}>
                ...
                <Link href={d.link ?? "#"} className="flex items-center px-2 py-3 text-neutral-400 hover:text-white" onClick={d.closeSideMenu}>
                    <span>{d.label}</span>
                </Link>
            )}
        </div>
